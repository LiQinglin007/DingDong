...
import com.li.xiaomi.xiaomilibrary.eventmessage.MiEventMessage;
import com.li.xiaomi.xiaomilibrary.utils.MiFinalData;
 ...
 * 这里有一点需要注意，如果有强制更新的概念，这时候请求数据的时候要给服务器你当前的版本号，
 * 如果你现在的版本号是1  ，用户很久没有操作，期间版本更新了2（强制更新）、3（不强制更新）。这时候用户打开版本号为1的版本，应该是要强制更新的。
...
public class UpdateAppUtils {
    ...
    public static void UpdateApp(Activity activity, Fragment fragment, int newVersionCode, String newVersionName,
        ...
        if (mVersionCode < serviceVersionCode) {//第2步骤
            if (PreferenceUtils.getInt(MiFinalData.VERSION_CODE, 0) < serviceVersionCode || IsToast) {//第3步骤
                //这时候要去更新，展示下载的对话框
                showDownLoadDialog();
            }
        } else {
                ...
                setNegativeButton("取消", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (IsUpdate) {
                            ...
                            PreferenceUtils.setInt(MiFinalData.VERSION_CODE, serviceVersionCode);
                            dialog.dismiss();
                        }
                    }
                }).
    ...
    private static void downLoad() {
        ...
        EventBus.getDefault().post(new MiEventMessage(MiEventMessage.CheckApp, true));
    ...
    private static void exitApp() {
        MiEventMessage mSumMessage = new MiEventMessage(MiEventMessage.ExitApp);
